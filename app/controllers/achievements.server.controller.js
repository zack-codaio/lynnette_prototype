 ...
 */
exports.list = function(req, res) { Achievement.find().sort('-created').populate('user', 'displayName').exec(function(err, achievements) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 */
exports.achievementByID = function(req, res, next, id) { Achievement.findById(id).populate('user', 'displayName').exec(function(err, achievement) {
